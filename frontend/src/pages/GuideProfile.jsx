...
export default function GuideProfile() {
  ...
  return (
    <div className="profile-page">
      ...
      />
      {/* Danger Zone - owner only (moved from header) */}
      {!isPublicView && (me.role === 'guide' || me.role === 'hiker') && (
        <div className="danger-zone" style={{ marginTop: 20, border: '1px solid #f5c6cb', background: '#fff5f6', padding: 16 }}>
          <h3 style={{ color: '#721c24', marginTop: 0 }}>Danger Zone</h3>
          <p style={{ color: '#721c24' }}>Deleting your account will remove your profile and anonymize personal data. This action is irreversible.</p>
          <button className="btn btn-danger" onClick={() => setDeleteConfirmOpen(true)} disabled={deleteInProgress}>
            {deleteInProgress ? 'Deleting…' : 'Delete Account'}
          </button>
          {deleteError && <div style={{ color: '#721c24', marginTop: 8 }}>{deleteError}</div>}
        </div>
      )}
      ...
      <ConfirmDialog
        ...
        message="Are you sure you want to permanently delete your account? This will remove your profile and anonymize personal data."
        requireText={true}
        requiredText={'DELETE'}
